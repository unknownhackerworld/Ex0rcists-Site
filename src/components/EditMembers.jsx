...
const EditMembers = () => {
  ...
  useEffect(() => {
    ...
    onValue(membersRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setMembersList(Object.keys(data));
      } else {
        setMembersList([]);
      }
    });
  ...
  // Load details when member is selected
  useEffect(() => {
    if (!selectedMember) return;
    const memberRef = ref(database, `members/${selectedMember}`);
      ...
      if (snapshot.exists()) {
        ...
        setSelectedTags(
          (data.Categories || []).map((cat) => {
            const tag = challengeTags.find((t) => t.label === cat);
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <select
          ...
          className="p-3 rounded-md bg-black text-white gradient-border w-full"
        >
