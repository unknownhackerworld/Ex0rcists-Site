...
const Members = () => {
  ...
  const filteredMembers = members.filter(
    (m) => selectedCategory === "All" || m.fields.includes(categories[selectedCategory])
  );

  const openModal = (member) => {
    setFlippedMember(member);
    setTimeout(() => {
  ...
  return (
    <>
      <style>{`
        .glitch-overlay { position: fixed; inset: 0; pointer-events: none; z-index: 60; }
        .glitch-bar {
          position: absolute; left: -5%; width: 110%; height: 6px;
          background: rgba(197, 4, 0, 0.12); transform: skewX(-10deg);
          animation: glitchBar 350ms linear infinite;
          mix-blend-mode: overlay; filter: saturate(1.3) blur(.2px);
        }
        @keyframes glitchBar {
        ...
        .flip-spin { animation: flipOnce 0.85s cubic-bezier(0.55, 0.3, 0.4, 0.9) forwards; }
        .modal-overlay {
          position: fixed; inset: 0; z-index: 70; display: flex;
          align-items: center; justify-content: center;
          background: rgba(0,0,0,0.55); backdrop-filter: blur(6px) saturate(1.05);
        }
        @keyframes modalFade {
          from { opacity: 0; transform: scale(0.9); }
          to { opacity: 1; transform: scale(1); }
        }
        .modal-card { animation: modalFade 0.6s ease-out both; }
      `}</style>
      ...
      <div className="flex md:flex-row flex-col w-full text-white px-8 py-4 gap-8 font-share">
        <aside className="hidden md:block flex-shrink-0 rounded-xl bg-[rgba(44,44,44,0.44)] p-6 sticky top-24 self-start">
          ...
          <ul className="space-y-8">
            <li className="relative flex items-center ml-14 text-gray-200 text-lg">
              <span className="absolute -left-10 w-8 h-[1px] bg-[#950C09]" />
              <button
                onClick={() => setSelectedCategory("All")}
                className={`px-2 py-1 transition-colors ${selectedCategory === "All" ? "text-white font-semibold" : "text-gray-300 hover:text-white"
                  }`}
              >
                All
              </button>
            ...
            {Object.entries(categories).map(([key, label]) => (
              <li key={key} className="relative flex items-center ml-14 text-gray-200 text-lg">
                <span className="absolute -left-10 w-8 h-[1px] bg-[#950C09]" />
                <button
                  onClick={() => setSelectedCategory(key)}
                  className={`px-2 py-1 transition-colors ${selectedCategory === key ? "text-white font-semibold" : "text-gray-300 hover:text-white"
                    }`}
                >
                  {label}
                </button>
        ...
        <div className="md:hidden w-full flex items-center gap-2 mb-4 relative">
          ...
          <div ref={scrollRef} className="flex gap-3 overflow-x-auto scrollbar-hide px-8">
            <button
              key="All"
              onClick={() => setSelectedCategory("All")}
              className={`flex-shrink-0 px-4 py-2 text-sm rounded-full border transition ${selectedCategory === "All"
      ...
      {isModalOpen && activeMember && (
        ...
        >
          <div className={`modal-card ${closing ? "closing" : ""}`} style={{ zIndex: 80, width: "min(92%, 1000px)" }}>
            <div className="bg-[#080808] rounded-2xl  p-6 md:p-10 flex flex-col md:flex-row gap-6 md:gap-8 items-stretch shadow-[0_0_20px_rgba(197,4,0,0.6)]">
              <div className="flex-1 text-left text-white pr-2">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={closeModal} aria-label="Close" className="ml-4 text-gray-300 hover:text-white">
                    ✕
                  </button>
                </div>
                <p className="mt-2 text-gray-400">@{activeMember.username}</p>
                <p className="mt-4 italic text-2xl text-[#C50400] font-serif">
                  {activeMember.tagline}
                </p>
                <p className="mt-6 text-lg text-gray-200 leading-relaxed">{activeMember.notes}</p>
                <div className="mt-6 flex items-center gap-4">
                  ...
                  >
                    {/* LinkedIn Icon */}
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 382 382" width="24" height="24" fill="currentColor" className="text-[#C50400]">
              ...
              <div className="flex-shrink-0 flex items-center justify-center md:justify-end">
                <div className="rounded-full p-[3px] bg-gradient-to-br from-[#C50400] to-[#5F0200]">
                  <div className="w-44 h-44 rounded-full overflow-hidden bg-black">
                    <img src={activeMember.picture} alt={activeMember.name} className="w-full h-full object-cover" />
                  </div>
                </div>
              </div>
            </div>
